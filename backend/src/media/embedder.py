...
class MediaEmbedder:
    ...
    def encode_video_keyframes(self, keyframes: List[Image.Image]) -> Tuple[np.ndarray, List[np.ndarray]]:
        ...
        try:
            ...
            # Weighted sum
            weighted_embedding = (
                frame_emb_tensor * attention_weights.unsqueeze(1)).sum(dim=0)
            ...
            logger.debug(
                f"Used attention-weighted pooling for {len(keyframes)} frames")

        except Exception as e:
            # Fallback to mean pooling if attention fails
            logger.warning(
                f"Attention-weighted pooling failed, using mean pooling: {e}")
